...
Note:
- This script will delete the git-cloned `.git` directory in the target directory, so you can start
...
def rename_files_and_directories(target_directory: Path, new_project_name: str, new_app_name: str) -> None:
    ...
    for item in target_directory.rglob('*'):
        assert isinstance(item, PosixPath)
        if item.is_dir():
...
def parse_args() -> None:
    ...
    parser.add_argument('--new_project_name', type=str, required=True,
...
def run_updater(target_directory: Path, new_project_name: str, new_app_name: str) -> None:
    ...
    print(f'Updated project and app references in ``{target_directory}`` to '
          '``{new_project_name}`` and ``{new_app_name}``.')
    return
