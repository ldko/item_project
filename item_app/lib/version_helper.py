...
class GatherCommitAndBranchData:
    ...
    - Now it reads the `.git/HEAD` file to get the commit and branch data, so it
      doesn't need to be asyncronous.
    """
    ...
    async def manage_git_calls(self):
        ...
        - Originally this class made two separate asyncronous subprocess calls to git.
        - Now it reads the `.git/HEAD` file to get both the commit and branch data
          (to avoid the `dubious ownership` issues), so it no longer benefits from
          asyncronous calls, but keeping for reference.
        - Called by views.version().
        """
    ...
    async def fetch_commit_data(self, results_holder_dct):
        """Fetches commit-data by reading the `.git/HEAD` file.

        Avoids calling git via subprocess due to `dubious ownership` issue.
    ...
    async def fetch_branch_data(self, results_holder_dct):
        """Fetches branch-data by reading the `.git/HEAD` file.
